...
function App() {
          ...
          {qItem.choices.map(choice => {
            return (
              <button
                type='button'
                key={choice.name}
                onClick={() => checkAnswer(choice.correct)}
              >
                {choice.name}
              </button>
            )
  ...
  const onStatsRender = () => {
    ...
    const correctRate = isNaN(game.stats.correctCount / total)
      ? 0
      : game.stats.correctCount / total
    const wrongCount = isNaN(game.stats.wrongCount / total)
      ? 0
      : game.stats.wrongCount / total
    return (
      <div className='onStats' style={{ textAlign: 'left' }}>
        ...
        <div>CORRECT : {(correctRate * 100).toFixed(0)}%</div>
        <div>WRONG : {(wrongCount * 100).toFixed(0)}%</div>
        <span>...</span>
  ...
  const onPauseRender = () => {
    return (
      <div className='onPause'>
        PAUSED!
        <span>...</span>
        <button type='button' onClick={() => unpauseGame()}>
          RESUME
        </button>
        <button type='button' onClick={() => resetGame()}>
          RESTART
  ...
  const onOverRender = () => {
    return (
      <div className='onOver'>
        {game.gameOverType === 'newHighScore' ? (
  ...
  const onStartRender = () => {
    return (
      <div className='onStart'>
        ...
        <button type='button' onClick={() => shareGame()}>
  ...
  if (game) {
    return (
      ...
      >
        ...
        <div className='container'>{renderPage(game.gameStatus)}</div>
