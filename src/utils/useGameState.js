...
const useGameState = () => {
  ...
  const [game, dispatch] = useReducer((state, action) => {
    const { highScore, timesPlayed, correctCount, wrongCount } = JSON.parse(
      localData,
    )
    switch (action.type) {
      default:
      ...
      case 'GAME_OVER':
        if (highScore < state.score.currentScore) {
          setLocalData(
            JSON.stringify({
              highScore: state.score.currentScore,
              timesPlayed: timesPlayed + 1,
              correctCount: correctCount + state.counter.correct,
              wrongCount: wrongCount + state.counter.wrong,
            }),
          )
          return {
            ...state,
      ...
      case 'CORRECT':
        return {
          ...
          timeEffect: {
            state: !state.timeEffect.state,
            color: { r: 22, g: 168, b: 22 },
          },
          score: {
            ...state.score,
            currentScore:
              state.score.currentScore + 10 * state.score.scoreMultiplier,
            streak: state.score.streak + 1,
            scoreMultiplier: calcMultiplier(
              state.score.streak,
              state.score.scoreMultiplier,
            ),
          },
          counter: {
            question: state.counter.question + 1,
            correct: state.counter.correct + 1,
            wrong: state.counter.wrong,
          },
        }
      case 'WRONG':
        return {
          ...
          timeEffect: {
            state: !state.timeEffect.state,
            color: { r: 239, g: 84, b: 84 },
          },
          score: { ...state.score, scoreMultiplier: 1, streak: 0 },
          counter: {
            question: state.counter.question + 1,
            correct: state.counter.correct,
            wrong: state.counter.wrong + 1,
          },
        }
    }
  })
